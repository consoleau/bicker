describe 'PendingViewCounter', ->
  ...
  it 'should fire the bicker_router.initialViewsLoaded event when the count first decreases to 0 and the ' +
    ...
    inject (PendingViewCounter, $rootScope) ->
      currentViewsLoadedListener = jasmine.createSpy 'bicker_router.currentViewsLoaded listener'
      $rootScope.$on 'bicker_router.currentViewsLoaded', currentViewsLoadedListener

      initialViewsLoadedListener = jasmine.createSpy 'bicker_router.currentViewsLoaded listener'
  ...
  it 'should fire the bicker_router.initialViewsLoaded event once',
    inject (PendingViewCounter, $rootScope) ->
      eventListener = jasmine.createSpy 'bicker_router.initialViewsLoaded listener'
